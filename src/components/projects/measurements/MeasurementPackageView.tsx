...
/**
 * O PACOTE DA MEDIÇÃO — prontidão, exigências, evidência vinculada, aceite e
 * histórico de UMA instância de `project_measurements`.
 *
 * Extraído da antiga aba de Medições sem uma linha de lógica alterada: o
 * conteúdo continua o mesmo, e o que mudou foi o dono. Agora ele é um PAINEL
 * do item de trabalho — aparece quando a instância canônica existe, e cede o
 * lugar à bancada de evidência quando ela ainda não foi materializada.
 *
 * ─── O que este painel continua recusando fazer ────────────────────────────
 *
 *   · Não mostra "R$ pronto para faturar". Direito de faturar é de Contratos.
 *   · Não apresenta evidência inferida como confirmada.
...
import {
  AlertTriangle, CheckCircle2, CircleSlash, FileCheck2, HelpCircle, ShieldQuestion,
} from 'lucide-react';
 ...
 */
export const STATE_TONE: Record<ReadinessState, { text: string; bg: string; label: string }> = {
...
export function MeasuredValue({ m }: { m: ProjectMeasurementRow }) {
  /*
...
export function MeasurementPackageView({ pkg }: { pkg: MeasurementPackage }) {
