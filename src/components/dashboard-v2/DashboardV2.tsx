...
export function DashboardV2() {
  ...
  const { refresh } = res;
  const reload = useCallback(() => { refresh(); refreshDecisionBadge(); }, [refresh]);
  ...
  useEffect(() => {
    const onFocus = () => {
      ...
      if (lastLoad.current && Date.now() - lastLoad.current > REFOCUS_MS) refresh();
    };
  ...
  return (
    <AxPage testId="dashboard-v2">
      {res.data ? <Board data={res.data} reload={reload} loading={res.state === 'loading'} />
...
function Board({ data, reload, loading }: { data: Payload; reload: () => void; loading: boolean }) {
  ...
  return (
    <div className="dv2" data-loading={loading ? 'true' : undefined}>
      <CommandHeader
        ...
        actions={<>
          ...
          <button type="button" className="ax-btn ghost icon" onClick={reload} aria-label="Recarregar a situação" title="Recarregar">
      ...
      <div className="dv2-board">
        ...
        <div className="dv2-slot dv2-slot-main ax-grid main-side">
          <AttentionFeed section={data.feed} today={data.today} readable={data.readable} hasOperation={data.hasOperation}
            apex={data.apex} onExplain={setExplain} />
...
function HeaderContext({ data }: { data: Payload }) {
  const feed = data.feed.state === 'ok' ? data.feed.data : null;
  ...
  return (
    <>
      {feed ? (
        <span><strong>{plural(feed.total, 'exceção', 'exceções')}</strong></span>
      ) : data.feed.state === 'restricted' ? <span>Fila de atenção restrita ao seu perfil</span> : null}
      {feed && feed.critical > 0 && (
        <span><Dot tone="danger" label="crítico" /><strong>{feed.critical}</strong> {feed.critical === 1 ? 'crítica' : 'críticas'}</span>
      )}
