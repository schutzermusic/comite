  ...
  if (!item) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-sm hud-text-muted">Selecione uma deliberação para ver os detalhes.</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto space-y-3 pr-1">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="ig-section-title">{item.title}</h2>
          <p className="text-sm hud-text-muted">{item.ownerCommitteeName}</p>
        </div>
      ...
      <Tabs defaultValue="summary" className="w-full">
        ...
        <TabsContent value="summary" className="space-y-3">
          <HUDCard className="space-y-2">
            <p className="text-xs uppercase tracking-wide hud-text-muted">Decisão Solicitada</p>
            <p className="text-sm hud-text-secondary">{item.requestedDecision || item.description}</p>
          </HUDCard>

          <HUDCard className="space-y-2">
            <p className="text-xs uppercase tracking-wide hud-text-muted">Fluxo de Governança</p>
            {(item.stages ?? []).map((stage) => (
              <div key={stage.id} className="flex items-center justify-between text-sm">
                <span className="hud-text-secondary">{stage.sequence}. {stage.committeeName}</span>
                <span className="hud-text-muted">{stageTypeLabel[stage.stageType] ?? stage.stageType}</span>
              </div>
            ))}
          </HUDCard>

          <HUDCard className="space-y-1">
            <p className="text-xs uppercase tracking-wide hud-text-muted">Etapa Atual</p>
            <p className="text-sm hud-text">{currentStage?.committeeName} - {stageTypeLabel[currentStage?.stageType ?? ''] ?? currentStage?.stageType}</p>
          </HUDCard>
        </TabsContent>
        ...
        <TabsContent value="voting" className="space-y-3">
          ...
          <HUDCard className="space-y-1">
            <p className="text-xs uppercase tracking-wide hud-text-muted">Resumo da Votação</p>
            <p className="text-sm hud-text-secondary">Sim {yes} | Não {no} | Abstenção {abstain}</p>
          </HUDCard>
        </TabsContent>
        ...
        <TabsContent value="minutes" className="space-y-3">
          <HUDCard className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-wide hud-text-muted">Atas</p>
            ...
            <p className="text-sm hud-text-secondary whitespace-pre-wrap">{item.minutesSummary || 'Minuta ainda não gerada.'}</p>
          </HUDCard>
        ...
        <TabsContent value="execution" className="space-y-3">
          <HUDCard className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-wide hud-text-muted">Ações Pós-Decisão</p>
            ...
            {(item.executionItems ?? []).length === 0 ? (
              <p className="text-sm hud-text-muted">Nenhuma ação de execução registrada.</p>
            ) : (
              (item.executionItems ?? []).map((task) => (
                <div key={task.id} className="rounded-lg border border-[rgba(255,255,255,0.1)] p-3 text-sm">
                  <p className="hud-text">{task.title}</p>
                  <p className="hud-text-muted">Responsável: {task.ownerName} | Prazo: {task.dueDate.toLocaleDateString()}</p>
                  <p className="hud-text-muted">Status: {taskStatusLabel[task.status] ?? task.status}</p>
