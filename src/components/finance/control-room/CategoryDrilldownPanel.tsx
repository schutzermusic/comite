...
export function CategoryDrilldownPanel({ filters }: CategoryDrilldownPanelProps) {
  const baseFilter: CategoryAnalysisFilter = useMemo(
    ...
    [filters.periodFrom, filters.periodTo, filters.projectId, filters.contractId, filters.costCenterId],
  );
  ...
  const drilldownEntries = useMemo(() => {
    ...
    return mockLedgerEntries
      .filter((e) => {
        ...
        if (baseFilter.costCenterId && baseFilter.costCenterId !== 'all' && e.cost_center_id !== baseFilter.costCenterId) return false;
        return true;
      })
  ...
  return (
    ...
    >
      ...
      <div data-ig-content="" className="flex flex-col">
        ...
        <div className="grid grid-cols-1 gap-px bg-[color:var(--ig-border-subtle)] lg:grid-cols-3">
          {/* Categorias */}
          <div className="bg-[color:var(--ig-bg-raised)]/40 p-3">
            <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[color:var(--ig-fg-subtle)]">Top Categorias</p>
            <div className="flex flex-col gap-0.5">
              {categories.length === 0 && <p className="px-2 py-6 text-center text-[11px] text-[color:var(--ig-fg-muted)]">Sem custos no período.</p>}
              {categories.slice(0, 8).map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center justify-between gap-2 rounded-md px-2 py-1.5 text-left transition-colors ${active ? 'bg-[color-mix(in_oklab,#14B8A6_14%,transparent)]' : 'hover:bg-[color:var(--ig-bg-hover)]'}`}
                  >
          ...
          {/* Subcategorias */}
          <div className="bg-[color:var(--ig-bg-raised)]/40 p-3">
            <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[color:var(--ig-fg-subtle)]">
              Subcategorias · {activeCategoryName}
            </p>
            <div className="flex flex-col gap-0.5">
              {subcategories.length === 0 && <p className="px-2 py-6 text-center text-[11px] text-[color:var(--ig-fg-muted)]">Selecione uma categoria.</p>}
          ...
          {/* Lançamentos */}
          <div className="bg-[color:var(--ig-bg-raised)]/40 p-3">
            <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[color:var(--ig-fg-subtle)]">Lançamentos</p>
            <div className="flex flex-col gap-0.5">
              {!selectedSubId && <p className="px-2 py-6 text-center text-[11px] text-[color:var(--ig-fg-muted)]">Selecione uma subcategoria.</p>}
              {selectedSubId && drilldownEntries.length === 0 && (
                <p className="px-2 py-6 text-center text-[11px] text-[color:var(--ig-fg-muted)]">Sem lançamentos no recorte.</p>
              )}
              ...
              {selectedSubId && drilldownEntries.length > 0 && (
                <div className="mt-1 flex items-center gap-1 px-2 pt-1 text-[10px] text-[color:var(--ig-fg-muted)]">
