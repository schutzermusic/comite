...
export function FiscalRegistrations() {
  ...
  const counts = { establishments: master.establishments.length, parties: master.recipients.length, services: master.services.length };
  ...
  return (
    <HudPageLayout>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <TabButton active={tab === 'parties'} onClick={() => { setTab('parties'); setShowForm(false); }} icon={<UsersRound />} label="Contrapartes" count={counts.parties} />
      ...
      <HudPanel title={tab === 'establishments' ? 'Estabelecimentos emitentes' : tab === 'parties' ? 'Perfil fiscal das contrapartes' : 'Catálogo de serviços'} headerActions={<HudButton size="sm" variant="primary" leftIcon={<Plus className="h-4 w-4" />} onClick={() => setShowForm((value) => !value)}>{showForm ? 'Fechar' : 'Adicionar'}</HudButton>}>
        {showForm && <form onSubmit={submit} className="mb-6 rounded-xl border border-ig-border bg-ig-panel p-4">
          ...
          {tab === 'parties' && <PartyFields recipients={master.recipients.map((entry) => ({ value: entry.id, label: `${entry.legal_name} · ${entry.document_normalized ?? 'sem documento'}` }))} />}
        ...
        {master.loading ? <p className="py-8 text-center text-sm text-ig-fg-muted">Carregando…</p> : tab === 'establishments' ? <EstablishmentList rows={master.establishments} /> : tab === 'parties' ? <PartyList rows={master.recipients} /> : <ServiceList rows={master.services} />}
      </HudPanel>
    </HudPageLayout>
  );
}
...
function PartyFields({ recipients }: { recipients: Array<{ value: string; label: string }> }) {
  return <div className="space-y-4">
    <p className="rounded-lg border border-ig-border bg-ig-panel-subtle p-3 text-xs text-ig-fg-muted">A identidade jurídica da contraparte é do cadastro canônico. Aqui você completa apenas o que a NFS-e exige: inscrição municipal e endereço fiscal.</p>
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3"><label className="flex flex-col gap-1.5"><span className="text-[11px] font-medium uppercase tracking-wider text-ig-fg-muted">Contraparte</span><select name="partyId" required className="h-10 rounded-lg border border-ig-border bg-ig-panel px-3 text-sm"><option value="">Selecionar…</option>{recipients.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}</select></label><NativeInput name="municipalRegistration" label="Inscrição municipal" /><NativeInput name="stateRegistration" label="Inscrição estadual" /><NativeInput name="email" label="E-mail fiscal" type="email" /><NativeInput name="phone" label="Telefone" /><NativeInput name="municipalityIbge" label="Código IBGE" /><NativeInput name="municipalityName" label="Município" /><NativeInput name="uf" label="UF" /><NativeInput name="countryCode" label="País ISO2" defaultValue="BR" /><NativeInput name="postalCode" label="CEP" /><NativeInput name="street" label="Logradouro" /><NativeInput name="streetNumber" label="Número" /><NativeInput name="district" label="Bairro" /></div>
  </div>;
}
...
function PartyList({ rows }: { rows: ReturnType<typeof useFiscalMasterData>['recipients'] }) {
  if (!rows.length) return <p className="py-8 text-center text-sm text-ig-fg-muted">Nenhuma contraparte cadastrada. O cadastro de contrapartes é do módulo canônico.</p>;
  return <div className="grid gap-3 md:grid-cols-2">{rows.map((row) => <div key={row.id} className="rounded-xl border border-ig-border p-4"><p className="font-semibold text-ig-fg-strong">{row.legal_name}</p><p className="mt-1 text-xs text-ig-fg-muted">{row.document_type ? `${row.document_type.toUpperCase()} ${row.document_normalized ?? '—'}` : 'Documento não informado'}</p><p className="mt-3 text-xs text-ig-fg-muted">{row.profile?.municipality_name ? `${row.profile.municipality_name}/${row.profile.uf}` : 'Perfil fiscal pendente'} · {row.profile?.email ?? 'sem e-mail'}</p></div>)}</div>;
}
