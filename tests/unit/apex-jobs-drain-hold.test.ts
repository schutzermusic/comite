...
describe('o que a trava NÃO faz', () => {
  ...
  it('não exige migration: é variável de ambiente, e o banco não sabe dela', async () => {
    ...
    // Este hotfix sai contra a ponta 167 da produção. Nada além dela.
    expect(migrations.at(-1)).toBe('167');
    for (const file of ['src/lib/platform/jobs/hold.ts', 'src/lib/platform/jobs/worker.ts']) {
      expect(readFileSync(file, 'utf8')).not.toContain('execution_job_id');
    }
  });
});
